...
contract PayrollProcessor {
    ...
    function makeTransfer(string memory _email, uint _amount, string memory _purpose) public returns (uint) {
        ...
        emit TransferMoney(_email,UserStore[msg.sender].email,_amount,_purpose,"ELT");
    }
    ...
    //     emit TransferMoney(_email,UserStore[msg.sender].email,_amount,_purpose,_currency);
    // }
    ...
    event TransferMoney(string email_receiver,string email_sender,uint amount,string purpose,string currency);
}